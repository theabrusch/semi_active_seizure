...
def main(args):
    ...
    # arguments for unlabeled datagenerator
    unlabeled_datagen = train_datagen.copy()
    unlabeled_datagen['bckg_stride'] = args.window_length
    unlabeled_datagen['seiz_stride'] = args.window_length
    train_datagen['bckg_rate'] = None
    train_datagen['anno_based_seg'] = False
    ...
    for subj in transfer_subjects:
        ...
        max_recs = int(np.min([args.max_recs, np.max([len(transfer_records[subj]['seiz']),
                               len(transfer_records[subj]['bckg'])])-1]))
        ...
        for i in range(max_recs):
            ...
            # Select the remaining records for unsupervised examples
            unlabeled_seiz = transfer_records[subj]['seiz'][(i+1):]
            unlabeled_bckg = transfer_records[subj]['bckg'][(i+1):]
            unlabeled_transfer_recs = dict()
            unlabeled_transfer_recs[subj] = np.append(unlabeled_seiz, unlabeled_bckg)

            # create labeled and unlabeled generators for training
            ...
            unlabeled_transfer_dataloader = get_generator.get_dataset_transfer_unlab(data_gen = train_datagen, 
                                                                                    subjs_to_use = [subj], 
                                                                                    batches_labeled = len(transfer_dataloader),
                                                                                    records_to_use = unlabeled_transfer_recs, 
                                                                                    split = 'train/'+subj + '_' + str(i+1), 
                                                                                    writer = None)
            ...
            loss_fn = get_loss.SemiSupervisedTransfer(class_loss, 
                                                      lambda_cons=args.lambda_cons,
                                                      lambda_rep = args.lambda_rep,
                                                      get_last_k = args.last_k,
                                                      epsilon_cons = args.epsilon_cons,
                                                      epsilon_rep = args.epsilon_rep)
            ...
            time = datetime.now()
            train_loss, val_loss = model_train.train_transfer_ssl(train_loader = transfer_dataloader,
                                                                    unlabeled_loader = unlabeled_transfer_dataloader,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lambda_rep', type=float, default=1)
    parser.add_argument('--epsilon_cons', type=float, default=0.7)
    parser.add_argument('--epsilon_rep', type=float, default=0.5)
